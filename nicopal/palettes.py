...
def pal_gallery(N=256, ncols=3, n_discrete=10, reverse=False):
    ...
    for idx, name in enumerate(palettes):
        ...
        ax.text(0.5, 0.16, name, transform=ax.transAxes, ha="center",
                va="center", fontsize=15, fontfamily="monospace",
                color="#3A3835")
...
def _plot_lightness(ax, cmap, N=256):
    ...
    t    = np.linspace(0, 1, N)
    ax.plot([0, 1], [L[0], L[-1]], color="gray", lw=0.8, ls="--",
            alpha=0.6, label="linear reference")
    for i in range(N - 1):
        ax.plot(t[i:i+2], L[i:i+2], color=rgba[i, :3], lw=2.5,
...
def _plot_discrete(ax, cmap, vmin, vmax, N=10):
    ...
    ax.bar(centers, np.ones(N), width=(vmax - vmin) / N * 0.92,
...
def _plot_cvd(ax, cmap, N=256):
    ...
    for i, (label, rgb) in enumerate(modes):
        ax.imshow(rgb[np.newaxis, :], aspect="auto",
                  extent=[0, 1, i, i + 0.85])
        ax.text(-0.02, i + 0.42, label, ha="right", va="center",
...
def pal_demo(name, vmin=-30, vmax=30):
    ...
    fig.suptitle(f"Nicopal demo — {base_name}",
                 fontsize=14, fontweight="bold", y=1)

    _plot_colorbar(axes[0, 0], cmap, vmin, vmax)
    _plot_signal_1d(axes[0, 1], cmap, vmin, vmax)
    _plot_lightness(axes[0, 2], cmap)
    _plot_field_2d(axes[1, 0], cmap, vmin, vmax)
    _plot_discrete(axes[1, 1], cmap, vmin, vmax)
    _plot_cvd(axes[1, 2], cmap)
